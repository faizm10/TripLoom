/* Dashboard — list of trips. Calm index page, not a wall of cards. */
...
const dashStyles = {
  ...
  row: {
    display:"grid",
    gridTemplateColumns: "44px 2.4fr 1fr 1fr 1fr 120px",
    alignItems:"center",
    gap: 16,
    padding: "18px 4px",
    borderBottom: "1px solid var(--hair)",
...
const Dashboard = ({ onOpenTrip }) => {
  ...
  const byYear = allTrips.reduce((acc, t) => {
    (acc[t.year] ||= []).push(t); return acc;
  }, {});
  const years = Object.keys(byYear).sort().reverse();

  return (
    <div style={dashStyles.page} data-screen-label="02 Dashboard">
      <aside style={dashStyles.side}>
        ...
        <div style={dashStyles.sideLink(true)}>
        ...
        <div style={dashStyles.sideLink(false)}>
          <Icon name="calendar" size={15} /> Upcoming
        </div>
        <div style={dashStyles.sideLink(false)}>
          <Icon name="bulb" size={15} /> Ideas
        </div>
        <div style={dashStyles.sideLink(false)}>
        ...
        {allTrips.slice(0,3).map(t => (
          <div key={t.id} style={dashStyles.sideLink(false)}>
      ...
      <main style={dashStyles.main}>
        <header style={dashStyles.header}>
          <div>
            <h1 style={dashStyles.h1}>Your trips</h1>
            <p style={dashStyles.sub}>Five in flight, one on the way, one to remember.</p>
          </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"44px 2.4fr 1fr 1fr 1fr 120px", gap: 16, padding:"0 4px" }}>
          <span></span>
          <span style={dashStyles.colHead}>Trip</span>
          <span style={dashStyles.colHead}>Dates</span>
          <span style={dashStyles.colHead}>People</span>
          <span style={dashStyles.colHead}>Places</span>
          <span style={dashStyles.colHead}>Status</span>
        </div>
        ...
        {years.map(year => (
          <section key={year} style={{ marginTop: 28 }}>
            <div style={{
              ...
              letterSpacing:"-0.01em", padding:"0 4px 8px", color:"var(--ink-3)"
            }}>{year}</div>
            {byYear[year].map(t => (
              <TripRow key={t.id} trip={t} onClick={() => t.id === "tokyo-2026" && onOpenTrip()} />
            ))}
          </section>
        ))}
      </main>
...
const TripRow = ({ trip, onClick }) => {
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <div style={dashStyles.cover}>
        <TripCover tag={trip.tag} seed={trip.seed || 1} compact />
      </div>
      <div>
        <div style={dashStyles.title}>{trip.title}</div>
        <div style={dashStyles.titleSub}>
          {trip.subtitle || `${trip.days} days · planned with ${trip.people} people`}
        </div>
      </div>
      <div style={dashStyles.stat}>{trip.when}</div>
      ...
      <div style={dashStyles.stat}>{trip.places} pins</div>
      <div><span style={dashStyles.statusChip(trip.status)}>{trip.status}</span></div>
    </div>
  );
};
