/* Shared primitives: brand mark, avatar stack, misc */

const Brand = ({ size = 18, withName = true }) => (
...
const LoomMark = ({ size = 22 }) => (
  <svg width={size} height={size} viewBox="0 0 32 32" fill="none"
       style={{ display:"block" }}>
    {/* Woven loom — three horizontal threads with two vertical warps */}
    ...
    <div style={{ display:"inline-flex", alignItems:"center" }}>
      ...
      {extra > 0 && (
        <div className={`avatar ${size}`}
             style={{ background:"var(--panel)", color:"var(--ink-3)", marginLeft:-6 }}>
          +{extra}
        </div>
      )}
    </div>
...
/* A muted illustrated placeholder for trip covers — each gets a distinct hue via tag var */
const TripCover = ({ tag = "tag-c", seed = 1, label, compact }) => {
  // Deterministic "shapes" built from seed
      ...
      <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none"
        ...
        {shapes.map((s, i) => (
          <circle key={i} cx={s.cx} cy={s.cy} r={s.r}
                  fill={i % 2 ? "rgba(22,24,28,.06)" : "rgba(255,255,255,.6)"} />
        ))}
...
Object.assign(window, { Brand, LoomMark, AvatarStack, PresenceDot, TripCover });
