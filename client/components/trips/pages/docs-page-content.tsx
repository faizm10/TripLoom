...
export function DocsPageContent() {
  ...
  React.useEffect(() => {
    ...
    async function fetchDocs() {
      try {
        const docs = await getTripDocuments(currentTripId)
        setDocuments(docs)
      } catch (error: unknown) {
        toast.error("Failed to load documents", { description: getErrorMessage(error) })
      } finally {
        setLoading(false)
      }
    }
  ...
  async function handleDownload(doc: TripDocument) {
    try {
      ...
      toast.error("Failed to generate download link", { description: getErrorMessage(error) })
    }
  }
